...
public class ChatActivity extends AppCompatActivity implements ChatroomsFragment.IChatroomListener, MessagesFragment.IChatListener, SendMessage.IMessageSender {
    ...
    public void send(String chatroom, String message) {
        ...
        ResultReceiverWrapper rr = new ResultReceiverWrapper(new Handler(Looper.getMainLooper()));
        rr.setReceiver(new ResultReceiverWrapper.IReceive() {
            @Override
            public void onReceiveResult(int resultCode, Bundle data) {
                if(resultCode == RESULT_OK){
                    Toast.makeText(ChatActivity.this, "Message was sent!", Toast.LENGTH_SHORT).show();
                } else {
        ...
        executor.execute( () -> {
            chatHelper.postMessage(chatroom,message, rr);
        });
